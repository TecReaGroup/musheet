...
class BackendService {
  ...
  Future<ApiResult<List<Score>>> getScores({DateTime? since}) async {
    if (_userId == null) return ApiResult(error: 'Not logged in');
    if (kDebugMode) print('[Backend] → getScores(user:$_userId, since:$since)');
    
    try {
      final result = await _client.score.getScores(_userId!, since: since);
      if (kDebugMode) print('[Backend] ✓ getScores → ${result.length} scores');
      return ApiResult(data: result, statusCode: 200);
    } catch (e) {
      if (kDebugMode) print('[Backend] ✗ getScores → $e');
      return ApiResult(error: e.toString());
    }
  }

  /// Sync a score to server
  Future<ApiResult<ScoreSyncResult>> syncScore(Score score) async {
    if (_userId == null) return ApiResult(error: 'Not logged in');
    if (kDebugMode) print('[Backend] → syncScore("${score.title}", id:${score.id})');
    
    try {
      final result = await _client.score.upsertScore(_userId!, score);
      if (kDebugMode) print('[Backend] ✓ syncScore → status:${result.status}, serverId:${result.serverVersion?.id}');
      return ApiResult(data: result, statusCode: 200);
    } catch (e) {
      if (kDebugMode) print('[Backend] ✗ syncScore → $e');
      return ApiResult(error: e.toString());
    }
  }

  /// Delete a score from server
  Future<ApiResult<bool>> deleteScore(int scoreId) async {
    if (_userId == null) return ApiResult(error: 'Not logged in');
    try {
      final result = await _client.score.deleteScore(_userId!, scoreId);
      if (kDebugMode) print('[Backend] ✓ deleteScore($scoreId)');
      return ApiResult(data: result, statusCode: 200);
    } catch (e) {
      if (kDebugMode) print('[Backend] ✗ deleteScore → $e');
      return ApiResult(error: e.toString());
    }
  }

  // ============== InstrumentScore Sync API ==============

  /// Get instrument scores for a score from server
  Future<ApiResult<List<InstrumentScore>>> getInstrumentScores(int scoreId) async {
    if (_userId == null) return ApiResult(error: 'Not logged in');
    
    try {
      final result = await _client.score.getInstrumentScores(_userId!, scoreId);
      if (kDebugMode) print('[Backend] ✓ getInstrumentScores($scoreId) → ${result.length}');
      return ApiResult(data: result, statusCode: 200);
    } catch (e) {
      if (kDebugMode) print('[Backend] ✗ getInstrumentScores → $e');
      return ApiResult(error: e.toString());
    }
  }

  /// Upsert instrument score on server
  Future<ApiResult<InstrumentScore>> upsertInstrumentScore({
    ...
    try {
      final result = await _client.score.upsertInstrumentScore(
        _userId!, scoreId, instrumentName,
        orderIndex: orderIndex, pdfPath: pdfPath,
      );
      if (kDebugMode) print('[Backend] ✓ upsertInstrumentScore($scoreId, $instrumentName) → id:${result.id}');
      return ApiResult(data: result, statusCode: 200);
    } catch (e) {
      if (kDebugMode) print('[Backend] ✗ upsertInstrumentScore → $e');
      return ApiResult(error: e.toString());
    }
  }

  /// Create instrument score on server (legacy)
  Future<ApiResult<InstrumentScore>> createInstrumentScore({
    required int scoreId,
    required String instrumentName,
    int orderIndex = 0,
  }) async {
    return upsertInstrumentScore(scoreId: scoreId, instrumentName: instrumentName, orderIndex: orderIndex);
  }

  /// Delete instrument score from server
  Future<ApiResult<bool>> deleteInstrumentScore(int instrumentScoreId) async {
    if (_userId == null) return ApiResult(error: 'Not logged in');
    try {
      final result = await _client.score.deleteInstrumentScore(_userId!, instrumentScoreId);
      if (kDebugMode) print('[Backend] ✓ deleteInstrumentScore($instrumentScoreId)');
      return ApiResult(data: result, statusCode: 200);
    } catch (e) {
      if (kDebugMode) print('[Backend] ✗ deleteInstrumentScore → $e');
      return ApiResult(error: e.toString());
    }
  }
  ...
  /// Get all setlists from server
  Future<ApiResult<List<Setlist>>> getSetlists() async {
    if (_userId == null) return ApiResult(error: 'Not logged in');
    try {
      final result = await _client.setlist.getSetlists(_userId!);
      if (kDebugMode) print('[Backend] ✓ getSetlists → ${result.length}');
      return ApiResult(data: result, statusCode: 200);
    } catch (e) {
      if (kDebugMode) print('[Backend] ✗ getSetlists → $e');
      return ApiResult(error: e.toString());
    }
  }

  /// Upsert setlist on server
  Future<ApiResult<Setlist>> upsertSetlist({
    required String name,
    String? description,
  }) async {
    if (_userId == null) return ApiResult(error: 'Not logged in');
    try {
      final result = await _client.setlist.upsertSetlist(_userId!, name, description: description);
      if (kDebugMode) print('[Backend] ✓ upsertSetlist("$name") → id:${result.id}');
      return ApiResult(data: result, statusCode: 200);
    } catch (e) {
      if (kDebugMode) print('[Backend] ✗ upsertSetlist → $e');
      return ApiResult(error: e.toString());
    }
  }

  /// Create a setlist on server (legacy)
  Future<ApiResult<Setlist>> createSetlist({required String name, String? description}) async {
    return upsertSetlist(name: name, description: description);
  }
