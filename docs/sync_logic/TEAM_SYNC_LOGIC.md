...
│  │  └── 后台静默下载，不阻塞用户操作                               │ │
│  │                                                                 │ │
│  │  Priority 3 (最低): 后台预加载队列                              │ │
│  │  ├── WiFi 环境下自动下载 Team 中未缓存的 PDF                    │ │
│  │  └── 可被高优先级任务打断                                       │ │
│  └────────────────────────────────────────────────────────────────┘ │
│                                                                      │
│  下载触发时机:                                                       │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │  1. 用户打开 TeamScore 详情页                                   │ │
│  │     → 预加载该 Score 所有 instrument 的 PDF                     │ │
│  │                                                                 │ │
│  │  2. 用户打开 Team Setlist 详情页                                │ │
...
│  │                                                                 │ │
│  │  4. 后台空闲时 (WiFi 环境)                                      │ │
│  │     → 遍历 Team 中所有 pdfSyncStatus='needsDownload' 的记录     │ │
...
│                                                                      │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │ 用户打开 Team 中的某个 Score                                    │ │
│  └───────────────────────────┬────────────────────────────────────┘ │
│                              │                                       │
│                              ▼                                       │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │ 获取该 Score 下用户选择的 TeamInstrumentScore                   │ │
│  └───────────────────────────┬────────────────────────────────────┘ │
│                              │                                       │
│                              ▼                                       │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │ 检查本地是否已有 pdfHash 对应的文件                             │ │
│  │ (可能来自个人库或其他 Team)                                     │ │
│  └───────────────────────────┬────────────────────────────────────┘ │
│                      ┌───────┴───────┐                              │
│                   已存在            不存在                           │
...
│  配置项:                                                             │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │  teamPdfAutoDownload: bool = true                               │ │
│  │  ├── true:  WiFi 下自动后台下载 Team PDF                        │ │
│  │  └── false: 仅按需下载，不自动预加载                            │ │
│  │                                                                 │ │
│  │  teamPdfPreloadCount: int = 3                                   │ │
│  │  ├── 进入 Setlist 时预加载前 N 个 Score 的 PDF                  │ │
│  │  └── 0 表示不预加载                                             │ │
│  │                                                                 │ │
│  │  teamPdfDownloadOnCellular: bool = false                        │ │
│  │  ├── true:  允许在移动网络下下载                                │ │
...
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │                                                                 │ │
│  │  needsDownload ──(下载中)──► downloading ──(完成)──► synced     │ │
│  │        │                           │                            │ │
│  │        │                           │                            │ │
│  │        └───────────(本地已有)──────┘                            │ │
│  │                                                                 │ │
