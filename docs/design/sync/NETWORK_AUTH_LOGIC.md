...
│                     Repository Layer                        │
│  ┌───────────────────────────────────────────────────────┐  │
│  │  数据源抽象：Local-first，Network 作为同步源；匿名模式不启用同步 │  │
│  └───────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────┤
│                     Network Layer                           │
│  ┌─────────────────────┐  ┌─────────────────────────────┐  │
...
| 本地数据变更（账户模式） | authenticated | 防抖后同步 |
| 应用恢复前台（账户模式） | connected + authenticated | 延迟增量同步 |
...
| 原则 | 说明 |
|------|------|
| **永不过期** | Token 不设置过期时间，长期有效 |
| **多设备共存** | 同一用户可在多设备同时登录，互不影响 |
| **密码独立** | 修改密码不影响已登录设备的 Token |

### 10.2 Token 格式

```
Access Token:  {userId}.{timestamp}.{random_hex_64}
Refresh Token: {userId}.{timestamp}.{random_hex_96}
```

- `userId`: 用户 ID，用于快速提取用户身份
- `timestamp`: 创建时间戳（仅记录，不用于过期判断）
- `random`: 安全随机数（hex 编码，避免特殊字符）
...
| 风险 | 缓解措施 |
|------|---------|
| Token 泄露 | 管理员可禁用账户，立即失效所有 Token |
| 设备丢失 | 用户可联系管理员禁用账户 |
| 长期未使用 | 应用层面可提示用户重新验证身份 |
...
| Token 永不过期 | ✅ 已实现 | 无过期检查 |
| 密码独立 | ✅ 已实现 | 修改密码不影响Token |

---

*文档版本: 3.2*
*更新日期: 2025-01-17*
*更新内容: 添加 Token 策略章节，明确 Token 永不过期和密码独立原则*
