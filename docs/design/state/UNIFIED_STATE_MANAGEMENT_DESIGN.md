...
管理应用的核心领域数据：乐谱列表、曲单列表、团队信息、用户认证状态、同步状态等。这些数据来自本地数据库或远程服务器，需要持久化存储。

在重构后的规范中，数据状态层进一步拆分为两类职责：

- **Query Provider（查询层）**：只负责暴露领域数据，不承担写操作职责。对于乐谱、曲单这类本地数据库可监听的数据，Query Provider 应优先直接基于数据库流构建，确保列表只有一个事实源。
- **Command Provider（命令层）**：只负责执行业务写操作，如新增、更新、删除、复制、导入、重排等。Command Provider 不再持有 `List<T>` 的权威状态，只暴露执行状态（如 submitting、error、lastAction）。

这种 Query/Command 分离的模式，用来避免同一个 Notifier 同时承担“列表状态”和“增删改命令”两种职责，降低状态分叉风险。
...
对于乐谱、曲单这类由本地数据库驱动的领域数据，不再推荐由 `Notifier<AsyncValue<List<T>>>` 直接维护一份长期存在的内存列表副本。统一机制如下：

**读路径**

Query Provider 直接监听数据库流，并将数据库中的数据作为唯一事实源暴露给 UI。

**写路径**

所有 CRUD、复制、导入、重排等动作统一由 Command Provider 发起，Command Provider 调用 Repository 完成持久化；数据库变化后，再由 Query Provider 自动把最新结果推送到 UI。

**是否乐观更新**

只有在确有交互收益且能严格保证一致性的场景，才允许在 Command Provider 中做局部乐观反馈；但乐观反馈不能成为长期主状态，更不能替代数据库流本身。绝大多数列表更新应遵循“写数据库 → 流式回推 UI”的方式。

这种模式的核心目的是确保：

- 列表状态来源唯一
- 写操作入口唯一
- UI 不需要判断应该直接改 state 还是等待数据库刷新
...
**禁止在 Screen 中直接读取 Repository 执行写操作**：Screen 只能订阅 Query Provider、读取 UI State Provider、调用 Command Provider 分发意图。

**禁止在兼容层 Provider 中继续新增业务方法**：历史兼容 Provider 可以在过渡期保留，但应冻结写接口，避免新的业务逻辑继续堆积在旧入口中。
...
**scores_state_provider.dart**：乐谱 Query Provider 与兼容层入口。长期目标是不再在该文件中堆积新的写操作。

**setlists_state_provider.dart**：曲单 Query Provider 与兼容层入口。

**score_commands_provider.dart / setlist_commands_provider.dart**：领域命令入口，负责新增、编辑、删除、复制、导入、重排等写操作。

**teams_state_provider.dart**：团队数据的状态管理；后续也可按 Query/Command 分离。

**ui_state_providers.dart**：所有 UI 状态的统一定义。禁止在 Screen 文件中重复定义同类 Provider。
...
本架构设计围绕"单一事实源、状态分层、数据域统一、读写分离"四个核心理念，构建了清晰的状态管理体系：

**分层架构**确保了职责分离，每一层专注于自己的工作。

**DataScope 机制**消除了 Library/Team 的代码重复，提供了一致的编程模型。

**Query/Command 分离**确保领域列表由数据库流统一驱动，写操作通过命令入口收口，避免状态分叉。
...
遵循本设计进行实现，可以有效解决当前代码中的状态管理问题，尤其适合逐步收敛当前 `score` / `setlist` 领域中多入口、双轨并存的历史问题，为 MuSheet 应用奠定坚实的技术基础。
