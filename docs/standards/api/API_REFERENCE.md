...
MuSheet 使用 Serverpod 构建后端，并通过 Serverpod Endpoint / RPC 方式提供接口能力。

当前实现与文档约定需要注意以下几点：

- 客户端主要通过 Serverpod 生成客户端调用 Endpoint，而不是传统 REST 路径直连。
- 认证相关接口包括 [`AuthEndpoint.register()`](server/musheet_server/lib/src/endpoints/auth_endpoint.dart:13)、[`AuthEndpoint.login()`](server/musheet_server/lib/src/endpoints/auth_endpoint.dart:84)、[`AuthEndpoint.refreshToken()`](server/musheet_server/lib/src/endpoints/auth_endpoint.dart:206) 等。
- 部分业务接口会通过传入 `userId` 后，再结合服务端鉴权辅助逻辑进行校验，例如 [`ScoreEndpoint.getScores()`](server/musheet_server/lib/src/endpoints/score_endpoint.dart:10)。
- 文件存储与头像等资源当前由服务端本地文件系统路径承载，例如 [`ProfileEndpoint.uploadAvatar()`](server/musheet_server/lib/src/endpoints/profile_endpoint.dart:82) 中的 `avatars/` 路径。
...
- **文件存储**: 服务端本地文件系统（如 `avatars/`、PDF 存储目录）
...
当前实现与 [`auth_result.yaml`](server/musheet_server/lib/src/protocol/dto/auth_result.yaml:1) 一致，注册与登录都会返回 `token` 与 `refreshToken`。
...
**冲突检测：** 使用乐观锁（`version` 字段），如果服务器版本号大于客户端版本号，返回冲突。

**当前实现补充：** 当客户端未携带 `id` 时，服务端仍会先按 `(scopeType='user', scopeId=userId, title, composer, deletedAt=null)` 查找已存在记录，再决定更新或创建，见 [`ScoreEndpoint.upsertScore()`](server/musheet_server/lib/src/endpoints/score_endpoint.dart:52)。
...
> 注意：当前实现中 [`createScore()`](server/musheet_server/lib/src/endpoints/score_endpoint.dart:141) 为直接创建接口，不包含 `upsertScore` 的唯一性合并逻辑；需要幂等同步时，应优先使用 `upsertScore`。
