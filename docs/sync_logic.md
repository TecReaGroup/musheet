...
| 方面 | v2.0 | v3.0 | 改进 |
|-----|------|------|------|
| **版本控制** | 每条记录 version + updatedAt | 全局 libraryVersion | 大幅简化 |
| **ID 系统** | id + serverId 双 ID | 仅 id | 消除复杂度 |
| **上传队列** | DELETE > CREATE > UPDATE 优先级 | 统一批量推送 | 逻辑简化 |
| **冲突检测** | 逐条比较时间戳 | 单一版本号 + 412 响应 | 无时钟依赖 |
| **API 设计** | 多个独立接口 | 统一 /sync 接口 | 维护简化 |
